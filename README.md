...
```mermaid
flowchart TD
    PROMPT(["User Prompt"]) --> GATE

    subgraph PIPELINE["ControlPlane.ai — LangGraph Orchestration Pipeline"]
        direction TB
        GATE["🛡️ Security Gate<br/><small>Presidio ML + DeBERTa v3 · PII + Injection (regex ⊔ ML)</small>"] --> SESSION["📈 Session Risk Tracker"]
        SESSION --> ROUTER{{"🧭 Semantic Router"}}
        ROUTER --> LLM[("🤖 LLM")]

        LLM --> D1 & D2 & D3 & D4 & D5 & D6

        subgraph DETECT["⚡ Parallel Detection (fan-out via asyncio)"]
            direction LR
            D1["PII in Response"]
            D2["Hallucination<br/><small>DistilBERT</small>"]
            D3["Bias<br/><small>BART zero-shot</small>"]
            D4["Retrieval Verifier"]
            D5["AI-as-Judge"]
            D6["Injection in Response"]
        end

        D1 & D2 & D3 & D4 & D5 & D6 --> FUSION["⚖️ Evidence Fusion Engine<br/><small>7-source weighted fusion</small>"]

        FUSION --> ALLOW["✅ ALLOW"]
        FUSION --> SANITIZE["🧹 SANITIZE"]
        FUSION --> REVIEW["🔎 REVIEW"]
        FUSION --> BLOCK["⛔ BLOCK"]

        REVIEW --> HITL["🧑‍⚖️ HITL Queue<br/><small>Human Approve / Reject / Edit / Regenerate</small>"]
    end
    ...
    HITL --> AUDIT
    BLOCK --> AUDIT
    AUDIT[("📜 Governance Audit Log + Auto-Threshold Tuner")]

    classDef gate fill:#1d4ed8,stroke:#1e3a8a,color:#fff
    classDef detect fill:#7c3aed,stroke:#5b21b6,color:#fff
    classDef fusion fill:#0891b2,stroke:#0e7490,color:#fff
    classDef allow fill:#16a34a,stroke:#15803d,color:#fff
    classDef sanitize fill:#ca8a04,stroke:#a16207,color:#fff
    classDef review fill:#ea580c,stroke:#c2410c,color:#fff
    classDef block fill:#dc2626,stroke:#991b1b,color:#fff
    classDef audit fill:#334155,stroke:#0f172a,color:#fff

    class GATE,SESSION,ROUTER,LLM gate
    class D1,D2,D3,D4,D5,D6 detect
    class FUSION fusion
    class ALLOW allow
    class SANITIZE sanitize
    class REVIEW,HITL review
    class BLOCK block
    class AUDIT audit
```

**How to read it:** the request path runs top to bottom through the **Security Gate** (blue) — every prompt is checked *before* the LLM is ever called. The response fans out to all six detectors in **Parallel Detection** (purple) simultaneously, so total added latency is roughly the slowest single detector, not the sum of six. Their outputs converge in the **Evidence Fusion Engine** (teal), which routes to exactly one of four outcomes: **ALLOW**/**SANITIZE** (green/amber, pass through), **REVIEW** (orange, parked in the HITL queue for a human), or **BLOCK** (red, rejected outright). Every path — including a human's HITL decision — ends at the same **Audit Log** (slate), so nothing bypasses the compliance trail.
